 ...
 * :set noexpandtab :%retab! :args ./*.ans :argdo %retab! |update (3) save and quit vim.
 */
public class AnswerFormatter {
    ...
    /**
     * 1.flink keeps NULLS_FIRST in ASC order, keeps NULLS_LAST in DESC order, choose corresponding
     * answer set file here. 2.for query 8、14a、18、70、77, decimal precision of answer set is too low
     * and unreasonable, compare result with result from SQL server, they can strictly match.
     */
    private static final List<String> ORIGIN_ANSWER_FILE =
            Arrays.asList(
                    "1",
                    "2",
                    "3",
    ...
    private static String formatRow(String row, List<Integer> colLengthList) {
        ...
        for (int i = 0; i < colLengthList.size(); i++) {
            ...
            start = start < row.length() ? start : row.length();
                    ...
                    (start + colLengthList.get(i)) < row.length()
                            ? (start + colLengthList.get(i))
                            : row.length();
    ...
    private static void writeContent(
            BufferedWriter writer, List<String> content, List<Integer> colLengthList)
            throws Exception {
